...
| `rate_limit` | `rate_limit`、`公平使用`、`frequency`、`429` | 从不 | 若错误含重置时间（如 `AccountQuotaExceeded`）：breakToShell + 定时 nudge（`重置时间 + 30s`）。否则让 claude 重试 |
...
1. 调用 `breakToShell` 停止 claude 的重试循环（C-c → claude 退出到 shell）
2. 通过 `setTimeout` 定时 nudge，时间为 `重置时间 + 30s`
3. 在 state 中记录 `next_nudge_at`（`cdog status` 表格中显示为 `next nudge` 列）
4. 定时器触发时：tmux 存活 → 直接 nudge；tmux 已死 → 用 `claude --resume` 重建 → nudge
