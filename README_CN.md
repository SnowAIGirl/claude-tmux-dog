...
| 9 | **自动关闭** | `max_run` 设置运行时长上限（如 `"7d"`），到期自动优雅停止 Agent。无需 cron——进程内 ms 级定时器 |
...
6. **恢复流程**——遇到可恢复错误，cdog 先输入 `cdog-recover` 标记、发送 Ctrl-C、检查标记是否存活，然后在同一 tmux 面板执行 `/new` + `claude --resume <session_id>`
...
`cdog init` 将 hook 脚本复制到 `~/.cdog/hooks/`，并将 `StopFailure` / `SessionStart` / `SessionEnd` hook 配置追加到 `~/.claude/settings.json`（会先备份为 `.cdog.bak`）。
...
| `provider` | `503`、`model_not_found`、`upstream error`、`overloaded_error`、`访问量过大` | 从不 | 让 claude 重试；8 次时通知 |
| `rate_limit` | `rate_limit`、`公平使用`、`429` | 从不 | 让 claude 重试；8 次时通知 |
| `unknown` | （未分类） | `threshold`（默认 3） | C-c → /context → compact-or-nudge |
...
| `interval` | `30` | 轮询间隔秒数（仅降级模式；pipe-pane 是事件驱动的） |
| `prompt` | `watchdog.prompt` | `/compact` 后发送的文本（延迟 10s，无需 C-c——claude 处于空闲状态） |
...
| 命令 | 说明 |
|------|------|
| `cdog start [name\|all]` | 在 detached tmux 会话中启动一个 Agent。缺少 hook 时自动执行 `cdog init` |
...
2. 如果错误是可恢复的（限流、过载、服务端错误、超时……）且熔断器未触发：先输入 `cdog-recover` 标记、发送 Ctrl-C、等待 1 秒、检查标记是否存活，然后在同一 tmux 面板执行 `/new` + `claude --resume <session_id>`
3. 如果错误不可恢复（认证失败、计费错误……）或熔断器已触发（5 分钟内 ≥3 次失败），将 Agent 标记为 `failed` 并停止恢复
...
本仓库内置 skill 定义，位于 `skills/cdog/`。任何 AI agent 都可以使用。全局安装 `claude-tmux-dog`（`npm install -g .`）并加载此 skill 后，agent 可在对话中直接管理后台 cdog 进程。skill 元数据：
